...
Timeout:1000
```
Расширение файлов конфигураций прописывать в имени не обязательно (то имя конфига может быть как conf.txt, так и просто conf)
## Использование
...
- Для получения списка всех доступных методов в интерйесе объекта используем команду
```bash
...
- Изначально в конфиге **confManagerApplication1** будут следующие параметры:
```bash
Timeout:1000
TimeoutPhrase:hello form aziz
``````
...
```
Теперь сообщение должно выводится каждые 5 секунд
####  Примечание
